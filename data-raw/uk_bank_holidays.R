...
##   2. alphagov/calendars GitHub repository   — 2012–2023, all three divisions
##   3. Manually compiled data:
##        England & Wales  : 2010–2014 and 2022–2026
##        Scotland         : 2022–2026
##        Northern Ireland : 2022–2026
##
## Where the same (date, division) appears in multiple sources, the
## highest-priority source wins.  Lower-priority sources extend coverage
## into date ranges not present in the higher-priority source.
##
...
# ---------------------------------------------------------------------------
# 3. Manually compiled data (lowest-priority backstop)
#     England & Wales  : 2010–2014 and 2022–2026
#     Scotland         : 2022–2026
#     Northern Ireland : 2022–2026
#
...
manual_dt <- data.table(
  date = as.Date(c(
    ...
    "2012-05-07",
    "2012-06-04",
    "2012-06-05", # Spring BH moved + Diamond Jubilee (extra)
    "2012-08-27",
    "2012-12-25",
    "2012-12-26",
    # 2013
    ...
    "2022-05-02",
    "2022-06-02",
    "2022-06-03", # Spring BH moved + Platinum Jubilee (extra)
    "2022-08-29",
    "2022-09-19", # State Funeral of Queen Elizabeth II (extra)
    "2022-12-26",
    "2022-12-27", # Dec 25 = Sunday: Boxing Day Mon, Christmas Tue
...
# ---------------------------------------------------------------------------
# 4. Combine all sources: gov.uk > alphagov > manual
#    For duplicate (date, division) pairs, the highest-priority source wins.
# ---------------------------------------------------------------------------

uk_bank_holidays <- manual_dt

if (!is.null(alphagov_dt) && nrow(alphagov_dt) > 0) {
  # alphagov supersedes manual: take all alphagov + manual rows not in alphagov
  uk_bank_holidays <- rbind(
    alphagov_dt,
    manual_dt[
      !paste(manual_dt$date, manual_dt$division) %in%
        paste(alphagov_dt$date, alphagov_dt$division)
    ]
  )
}

if (!is.null(govuk_dt) && nrow(govuk_dt) > 0) {
  # gov.uk supersedes everything: take all gov.uk + remaining rows not in gov.uk
  uk_bank_holidays <- rbind(
    govuk_dt,
    uk_bank_holidays[
