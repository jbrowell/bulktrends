...
comcode_plot <- function(
  ...
  if (import_data[, length(unique(COMCODE))] > max_unique_comcodes) {
    topX <- import_data[, .(variable = sum(get(variable))), by = COMCODE][
      order(variable, decreasing = T),
      COMCODE
    ][1:max_unique_comcodes]
  ...
  p <- ggplot(
    import_data,
    aes(x = get(date_col), y = variable, fill = factor(COMCODE))
  ) +
    # geom_area() +
    ...
    scale_fill_manual(
      values = colorRampPalette(c(
        ...
        "#6baed6",
        "#08306b"
      ))(length(unique(import_data$COMCODE)))
    ) +
    labs(
      title = if (variable == "volume") {
        ...
        if (nchar(code) >= 2) {
          import_data[
            ...
            paste(
              Hs2Code,
              "-",
              stringr::str_wrap(Hs2Description, width = 85),
              "\n"
            )
          ]
        } else {
        ...
        if (nchar(code) >= 4) {
          import_data[
            ...
            paste(
              Hs4Code,
              "-",
              stringr::str_wrap(Hs4Description, width = 85),
              "\n"
            )
          ]
        } else {
        ...
        if (nchar(code) >= 6) {
          import_data[
            ...
            paste(
              Hs6Code,
              "-",
              stringr::str_wrap(Hs6Description, width = 85),
              "\n"
            )
          ]
        } else {
        ...
        if (nchar(code) == 8) {
          import_data[
            ...
            paste(
              COMCODE,
              "-",
              stringr::str_wrap(Cn8LongDescription, width = 85),
              "\n"
            )
          ]
        } else {
