...
imports_ipaffs <- read_ipaffs(path = paste0(data_directory,"imports_ipaffs/non-animals/201"))
head(imports_ipaffs)
```

```{r}
#example of updated functions for monthly and daily data
ts_data <- extract_ts(imports, "month", "35", quantity = "NET_MASS")
head(ts_data)

ts_data <- extract_ts(imports_ipaffs, "date", "03", quantity = "weight")
head(ts_data)
