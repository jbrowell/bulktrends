...
Examples of how to use the `bulktrends` package (version `r packageVersion("bulktrends")`). It can be installed from it's [GitHub repository](https://github.com/jbrowell/bulktrends/) using
```{r installation}
#| eval: false
devtools::install_github("jbrowell/bulktrends")
```
Next, let's load the package and set the path to where we are storing data.
```{r set-up}
#| eval: false
library(bulktrends)
