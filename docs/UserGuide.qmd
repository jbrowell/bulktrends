...
Large-scale anomaly detection can be performed with `detect_anomalies()` using data in tsibble format.
`uktrade_tsibble()` converts a `data.table` into a `tsibble`: a time-series table format that supports later analysis, including the hierarchical forecasting algorithm.
