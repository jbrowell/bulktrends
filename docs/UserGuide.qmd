...
Parallel computation is supported in `bulktrends` using on the [`future` framework](https://future.futureverse.org/). Its use is optional but recommended. To use parallel computation, simply set a "plan".
```{r future_setup}
#| warning: false
...
#| include: false
if ( !exists("imports") ){
  if (file.exists(file.path(data_directory,"imports.rds"))) {
...
Detecting anomalies can be performed for a list of commodity codes using the `detect_anomalies()` function. The data to be supplied can either be lists of single time series or a `tsibble` data frame, a time-series table format that supports later analysis. The function returns (i) a table of all detected anomalies (breaks and outliers) across all codes, (ii) a `data.table` for each code containing the original time series, the time index, and the associated anomaly covariates and, (iii) the final model formula for each time series.
...
data_table_list <- extract_ts(imports, code = hs2_codes[1:5], return_list = T)

hs2_anomalies <- detect_anomalies(data = ts_list,
                                  response_col = "NET_MASS",
                                  scale_ts = TRUE,
                                  verbose = FALSE,
                                  tso_params = list(cval = 4, types = c("AO", "TC", "IO")))
...
tsibble_data <- uktrade_tsibble(
  imports[DATE_START >= "2024-01-01"],
  comcode_level = c("HS2", "HS4"),
  group_by = "category"
)

hs2_anomalies <- detect_anomalies(data = tsibble_data,
                                  response_col = "NET_MASS",
                                  scale_ts = TRUE,
                                  verbose = FALSE,
                                  tso_params = list(cval = 4, types = c("AO", "TC", "IO")))
