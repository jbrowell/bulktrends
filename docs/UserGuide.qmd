...
```
Next, let's load the package and add the path to where we are storing data.
```{r set-up}
#| warning: false
#| message: false
library(bulktrends)
...
-   Low value Trade EU: Imports via port code QVV only report chapter of `COMCODE` and do not include `NET_WEIGHT`.
