...
selected_model <- select_best_model(ts_data, scale_ts = T)
...
## Examples using IPAFFS

The Food Standards Agency makes some Import of Products, Animals, Food and Feed System (IPAFFS) data public which we can also analyse with `bulktrends`.

```{r}
devtools::load_all("../")
imports_ipaffs <- read_ipaffs(path = paste0(data_directory,"imports_ipaffs/Non-animal (FNAO)/2017"))
...
```{r}
select_best_model(ts_data, response_col = "NET_MASS", scale_ts = T)
```

```{r}

#anomaly detection for individual comcode
...
#numeric vector vs ts object
#y <- ts(as.numeric(scale(ts_data[["NET_MASS"]])))

detect_anomaly <- tso(y = as.numeric(scale(ts_data[["NET_MASS"]]))
                     cval=5,
                     types = c("AO", "LS", "TC", "IO"),
                      xreg = selected_model$xreg)
...
```{r}
#large scale anomaly detection for imports_ipaffs
